...
func (r *router) addRoute(method string, path string, handler HandleFunc, ms ...Middleware) {
	...
	for _, s := range segs {
		...
		root = root.childOrCreate(s, ms...)
	}
	if root.handler != nil && ms == nil {
...
func (n *node) childOrCreate(path string, ms ...Middleware) *node {
	if path == "*" {
		if n.paramChild != nil {
...
//
//func (m *matchInfo) startMdls() {
//	if m.mdls != nil {
//		for i := len(m.mdls) - 1; i >= 0; i-- {
//			m.mdls[i](m.n.handler)
//		}
//
//	}
//}
