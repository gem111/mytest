...
type Context struct {
	...
	TplEngin TemplateEngine
}

// Render 没有必要使用 statuscode进行传值 因为大部分都是直接返回200  这种设计代码非常清晰明了，保持住了respdata的语义，意味着中间件可以修改这个页面，列入直接替换为错误页面
//使用 io.write  不能使用中间件进行修改  所有使用这种
func (c *Context) Render(tplName string, data any) error {

	var err error
	data, err = c.TplEngin.Render(c.Req.Context(), tplName, data)
	if err != nil {
